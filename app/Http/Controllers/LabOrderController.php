...
public function getMyPrescriptions()
{
    ...
    if (!$patient) {
        return response()->json([
            'success' => false,
            'message' => 'الحساب الحالي ليس لمريض.'
        ], 403);
    }

    // جلب الوصفات الطبية (الراشيتات) المرتبطة بمواعيد هذا المريض
    $prescriptions = Prescription::whereHas('appointment', function ($q) use ($patient) {
            $q->where('patient_id', $patient->id);
        })
        ->with([
            'appointment.doctor', // اختياري: جلب اسم الطبيب الذي كتب الراشيتة
            'medicines'           // اختياري: جلب قائمة الأدوية (إذا كان لديك علاقة لها)
        ])
        ->orderBy('created_at', 'desc')
        ->get();
